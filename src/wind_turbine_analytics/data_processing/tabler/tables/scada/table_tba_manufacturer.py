"""TBA Manufacturer Tabler for SCADA analysis."""

from typing import Dict, Any, List
from src.wind_turbine_analytics.data_processing.tabler.base_tabler import BaseTabler


class TbaManufacturerTabler(BaseTabler):
    """
    Generates Time-Based Availability (TBA) Manufacturer table.

    Structure: Pivot table with months in rows and turbines in columns.
    - Each cell shows the manufacturer availability percentage for that month/turbine
    - Summary rows: MEAN (average) and STATUS (status level)

    TBA Manufacturer excludes certain downtimes (owner intervention, curtailment, icing)
    from unavailability calculation, similar to EbaManufacturerTabler logic.
    """

    def __init__(self):
        super().__init__(table_name="tba_manufacturer_table")
